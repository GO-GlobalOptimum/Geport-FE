...
const TagModal_update = ({ handleTagModalClose, handleSubmit }) => {
    ...
    return (
        <div style={{ position: 'absolute', top: 'calc(5% + 10px)', left: '90%', transform: 'translateX(-50%)', width: '15%', background: '#A3F0CBC3', padding: '20px', boxShadow: '0px 4px 10px rgba(0, 0, 0, 0.1)', borderRadius: '15px', zIndex: 999 }}>
            ...
            <div style={{ marginTop: '20px', textAlign: 'center' }}>
                <button onClick={handleFinalSubmit} style={{ background: '#00CC81', color: 'black', padding: '10px 20px', borderRadius: '15px', border: 'none', cursor: 'pointer' }}>게시물 수정하기</button>
            </div>
        </div>
    );
};

export default TagModal_update;
