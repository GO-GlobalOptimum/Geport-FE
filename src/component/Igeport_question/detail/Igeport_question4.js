...
export function Igeport_question4({ nextPage }) {

    const options = [
        '나는 사회적 활동에 참여하는 것을 피하곤 한다.',
        '나는 타인과의 상호작용에서 활력을 얻는다.'
    ];
    ...
    useEffect(() => {
        const savedOption = getCookie('Igeport_selectedOption4');
        if (savedOption) {
    ...
    const handleRadioChange = (value) => {
        ...
        setCookie('Igeport_selectedOption4', value, { path: '/' }); // 저장 위치와 경로 설정
    };
